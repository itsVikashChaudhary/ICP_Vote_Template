...
thread_local! {
    ...
    static PROPOSAL_MAP:RefCell<StableBTreeMap<u64,Proposal,Memory>> = RefCell::new(MemoryManager::init(StableBTreeMap::init(MEMORY_MANAGER.with(|m| m.borrow().get(MemoryId::new(0))))));
}
...
fn create_proposal(key: u64, proposal: CreateProposal) -> Option<Proposal> {
    ...
    PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal>>| p.borrow_mul().insert(key, value))
}
...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal>>| {
        ...
        match old_proposal_opt {
            Some(old_roposal) => old_proposal = value,
            None => return Err(VoteEror::NoSuchProposal),
        }
        if old_proposal.owner != ic_cdk::caller() {
            return Err(VoteEror::AccessRejected);
        }
        let value = Proposal {
            description: proposal.description,
            approval: old_proposal.approve,
        ...
        let res: Option<Proposal> = p.borrow_mul().insert(key, value);

        match res {
            Some(_) => OK(()),
            None => Err(VoteEror::UpdateError),
        }
    })
}
...
fn end_proposal(key: u64) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal>>| {
        ...
        let res: Option<Proposal> = p.borrow_mul().insert(key, old_proposal);
...
fn vote(key: u64, choice: Choice) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p: &RefCell<BTreeMap<u64, Proposal>>| {
        ...
        if proposal.voted.contains(&caller) {
            return Err(VoteEror::AlreadyVoted);
        ...
        let res = p.borrow_mul.insert(key, proposal);
        match res {
            some(_) => OK(()),
